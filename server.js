...
function sendMessageWithSmile(messageObject, socket) {
  ...
  server.clients.forEach(function each(client) {
    if (client.readyState === ws.OPEN) {
      messageObject.data = messageObject.data.replace(/:y/gi, ' 🙆‍♀️');
      messageObject.data = messageObject.data.replace(/:n/gi, '🙅');
      messageObject.data = messageObject.data.replace(/:h/gi, '🙋‍♀️');
      messageObject.data = messageObject.data.replace(/:omg/g, '🤦');
      messageObject.data = messageObject.data.replace(/:))/gi, '🤣');
      messageObject.data = messageObject.data.replace(/:*)/gi, '😘');
      messageObject.data = messageObject.data.replace(/:0/gi, '😮');
      messageObject.data = messageObject.data.replace(/:?)/gi, '🤔');
      messageObject.data = messageObject.data.replace(/:p/gi, '😜');
